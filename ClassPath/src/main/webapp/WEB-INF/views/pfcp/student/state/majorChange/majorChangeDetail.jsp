	...
	<div class="card">
		<form method="post" action="/student/counsel/majorChangeUpdate.do">
			...
			<div class="form-row" style="display: flex; gap: 20px;">
				...
				<div class="form-group" style="flex: 1;">
					<label class="inputLabel" for="statusComment">답변</label>
					<textarea class="textareaField" name="statusComment" rows="" cols="" readonly>${majorChange.statusComment }</textarea>
				</div>
			</div>
			...
			<div class="form-row" style="display: flex; gap: 20px;">
				<div class="form-group" style="flex: 1;">
					...
					<c:if test="${majorChange.status eq '대기중' }">
						<button class="submitButton" type="submit">저장</button>
						<button class="deleteButton" type="button" onclick="if(confirm('정말 삭제하시겠습니까?')) location.href='/student/counsel/majorChangeDelete.do?no=${majorChange.requestNo}'">삭제</button>					
					</c:if>
				</div>
				<div class="form-group" style="flex: 1;">
				</div>
