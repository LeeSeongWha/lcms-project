...
<div>
	<div>
		<form method="post" enctype="multipart/form-data" action="/student/scholarship/scholarshipUpdateProcess.do">
			...
			<div>
				<label class="inputLabel" for="approveDate">처리 일자</label>
				<input type="text" class="inputField" id="approveDate" name="approveDate" value="${scholarship.approveDate }" readonly>
			</div>
			<c:if test="${scholarship.applyStatus eq '반려'}">
				<div>
					<label class="inputLabel" for="approveDate">반려 사유</label>
					<input type="text" class="inputField" id="approveDate" name="approveDate" value="${scholarship.applyComment }" readonly>
				</div>
			</c:if>
			<div>
			...
			<div>
				<button type="button" onclick="history.back()" class="cancelButton" style="margin-top:20px;">목록으로</button>
				<c:if test="${scholarship.applyStatus eq '대기중'}">
					<button type="submit" class="submitButton">저장</button>
					<button type="button" onclick="if(confirm('정말 삭제하시겠습니까?')) location.href='/student/scholarship/scholarshipDelete.do?no=${scholarship.applyNo}'" class="deleteButton">삭제</button>				
				</c:if>
			</div>
		</form>
	</div>
