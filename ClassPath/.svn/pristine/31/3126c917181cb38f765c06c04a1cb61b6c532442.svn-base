...
<%-- 			<c:set var="postDateStr"> --%>
<%-- 			    <fmt:formatDate value="${board.postDate}"/> --%>
<%-- 			</c:set> --%>
<%-- 			<c:set var="updateDateStr"> --%>
...
<%-- 			</c:set> --%>
			...
			<div class="metaInfo" style="margin-left: auto; display: flex; flex-direction: column; text-align: right; font-size: 0.95rem; color: #555;">
			    ...
			    <p style="margin: 0; margin-top: 4px;">
			        <strong>작성일시:</strong> ${board.postDate}
			    </p>
			    <c:if test="${board.postDate ne board.updateDate}">
			        <p style="margin: 0; margin-top: 4px;">
			            <strong>수정일시:</strong> ${board.updateDate}
			        </p>
			    </c:if>
			</div>
		</div>
...
<%-- 		<h4 class="pageTitle" style="align: center" <c:if test="${editable}">hidden</c:if>>${board.boardTitle }</h4> --%>
		...
		<div class="button-group" style="display: flex; justify-content:space-between;">
			<a href="/staff/notice/noticeList.do" class="cancelButton" <c:if test="${editable}">hidden</c:if> style="padding: 10px 20px; border-radius: 4px;  min-width: 80px">목록으로</a>
