		...
		<form method="post" action="/student/counsel/returnApplyUpdate.do">
			...
			<div class="form-row" style="display: flex; gap: 20px;">
				<div class="form-group" style="flex: 1;">
					...
					<c:if test="${returnApply.status eq '대기중' }">
						<button class="submitButton" type="submit">저장</button>
						<button class="deleteButton" type="button" onclick="if(confirm('정말 삭제하시겠습니까?')) location.href='/student/counsel/returnApplyDelete.do?no=${returnApply.requestNo}'">삭제</button>					
					</c:if>
				</div>
				<div class="form-group" style="flex: 1;">
				</div>
